...
CREATE TABLE IF NOT EXISTS chapters (
  ...
  project_id UUID REFERENCES projects(id) ON DELETE CASCADE NOT NULL,
  title TEXT NOT NULL,
  content TEXT DEFAULT '', -- Changed from JSONB to TEXT to match existing schema
  summary TEXT,
...
CREATE INDEX IF NOT EXISTS chapters_order_index_idx ON chapters(order_index);
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can view own chapters'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'chapters' 
    AND policyname = 'Users can insert own chapters'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'chapters' 
    AND policyname = 'Users can update own chapters'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'chapters' 
    AND policyname = 'Users can delete own chapters'
  ) THEN
